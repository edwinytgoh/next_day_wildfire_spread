...
NPZ_DIR = "/dev/shm/next_day_wildfire_spread_npz"
...
def main():
    # Initialize experiment trackers (Weights and Biases and Tensorboard)
    if USE_WANDB:
        wandb.init(
    ...
    # Create loss function with weighted positive class, since we have very few positive pixels
...
def create_data_loaders(npz_dir, use_transforms=False):
    # Taken from: https://pytorch.org/vision/0.18/auto_examples/transforms/plot_transforms_e2e.html#
...
def run_one_epoch(
    ...
    with context:
        ...
        # # metrics["roc_auc"].update(valid_outputs, valid_targets.long())
...
def process_outputs_targets(outputs, targets):
    """Process model outputs and targets to ignore unlabeled pixels and remove singleton channel dims."""
    # Squeeze channel dim
    outputs = outputs["out"].squeeze(1)
    targets = targets.squeeze(1)

    # Apply mask and update metrics only for valid (non-masked) outputs and targets
    mask = targets != -1
    if mask.any():
